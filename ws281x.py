...
class LEDstrip:
    def __init__(self, nLEDs:int, LEDpin:int):
        # colors are coded as GRB intensity triplets
        pass
...
if __name__ == '__main__':
    ...
    strip.begin()
    try:
        color = Color(color="white", intensity=1#000000#000000#1A1A1A#1A1A1A#1A1A1A0)
        print('running')
        while True:
            for k in range(1, strip.numPixels()):
                strip.setPixelColor(k, color.getColorCode())
            strip.show()
            #time.sleep(0.1)
